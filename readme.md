...
New OTP verification replaces the old device.

2. One device cannot login with multiple mobile numbers.

3. admin action buttons should include: Mobile Number, Name, Registered Device, Last Login Time, IP/City approx, Remove Device, Block/Unblock Number.



For volatility notification, trigger notification when Silver MCX Buy moves ₹550 within 40 seconds 

Send notification once, then pause notification for 10 minutes. Notification language should be the user's set language. There is also an issue with the volatility message. The admin panel shows 'volatility triggered' in the changelog, but in the website/app there is no message of high volatility. please also fix this issue.
