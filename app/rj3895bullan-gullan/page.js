...
function ToggleRow({ label, checked, onChange }) {
  ...
  const unauthorizedLoginRecords = loginRecords.filter(
    ...
    {showLoginRecords
      ...
      : `Show Login Records${
          unauthorizedLoginRecords.length
            ? ` (${unauthorizedLoginRecords.length} unauthorized)`
            : ""
        }`}
  </button>
    ...
    <>
      ...
      {unauthorizedLoginRecords.length ? (
        <div style={styles.unauthorizedAlert}>
          ⚠️ Unauthorized access attempts found: {unauthorizedLoginRecords.length}
        </div>
      ) : null}
