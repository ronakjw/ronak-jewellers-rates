...
export default function SecurityAdminPage() {
  ...
  async function loadDevices(searchTerm = debouncedSearch) {
    ...
    try {
      ...
      const data = await apiFetch(`/api/device-manager${queryString ? `?${queryString}` : ""}`, token);
      if (data.success) setDevices(data.devices || []);
    } catch (err) {
      setMessage(err.message || "Unable to load devices");
    } finally {
  ...
  async function loadTargetAlerts() {
    ...
    try {
      const token = await getToken();
      const data = await apiFetch("/api/target-alerts/admin", token);
      if (data.success) setTargetAlerts(data.alerts || []);
    } catch (err) {
      setMessage(err.message || "Unable to load target alerts");
    } finally {
  ...
  async function loadFeedback() {
    ...
    try {
      const token = await getToken();
      const data = await apiFetch("/api/feedback/admin", token);
      if (data.success) setFeedback(data.feedback || []);
    } catch (err) {
      setMessage(err.message || "Unable to load feedback");
    } finally {
  ...
  async function loadNotificationSettings() {
    ...
    try {
      ...
      setMessage(err.message || "Unable to load notification settings");
    } finally {
  ...
  useEffect(() => {
    if (user?.email === ADMIN_EMAIL) loadDevices(debouncedSearch);
  ...
  async function removeTargetAlert(id) {
    ...
    try {
      ...
      if (!data.success) {
        throw new Error(data.message || "Unable to remove alert");
      }

      setTargetAlerts((prev) => prev.filter((alert) => alert.id !== id));
      setMessage("Target alert removed.");
    } catch (err) {
      ...
      <section style={styles.shell}>
        <div style={styles.header}>
          ...
          <div style={styles.headerActions}>
            ...
            <button style={styles.smallButton} onClick={loadAll} disabled={loading || deviceLoading || alertLoading || feedbackLoading || notificationLoading}>{loading || deviceLoading || alertLoading || feedbackLoading || notificationLoading ? "Loading..." : "Refresh"}</button>
        ...
        <section style={styles.card}>
          ...
          <h3 style={styles.sectionSubTitle}>{search.trim() ? "Search Results" : "Last 10 Login Devices"}</h3>
          {deviceLoading ? <p style={styles.subtitle}>Loading devices...</p> : null}
            ...
            {filteredDevices.map((item) => (
              <div key={item.phone} style={styles.deviceCard}>
                ...
                <div style={styles.actions}>
                  ...
                  {item.blocked ? (
                    <button style={styles.dangerSoftButton} disabled={Boolean(actionLoading[`device-${item.phone}`])} onClick={() => deviceAction(item.phone, "unblock")}>Unblok</button>
                  ) : (
                    <button style={styles.dangerButton} disabled={Boolean(actionLoading[`device-${item.phone}`])} onClick={() => deviceAction(item.phone, "block")}>Block</button>
                  )}
                </div>
              </div>
            ))}
        ...
        <section style={styles.card}>
          ...
          {notificationLoading && !notificationSettings ? <p style={styles.subtitle}>Loading notification settings...</p> : null}
        ...
        <section style={styles.card}>
          <h2>Target Alerts</h2>
          {alertLoading ? <p style={styles.subtitle}>Loading target alerts...</p> : null}
          {!alertLoading && targetAlerts.length === 0 ? <p style={styles.subtitle}>No active target alerts.</p> : null}
          {targetAlerts.map((alert) => (
            <div key={alert.id} style={styles.rowCard}>
              ...
              <button style={styles.dangerButton} disabled={Boolean(actionLoading[`alert-${alert.id}`])} onClick={() => removeTargetAlert(alert.id)}>Remove Alert</button>
            </div>
          ))}
        </section>
        ...
        <section style={styles.card}>
          <h2>Feedback</h2>
          {feedbackLoading ? <p style={styles.subtitle}>Loading feedback...</p> : null}
          {!feedbackLoading && feedback.length === 0 ? <p style={styles.subtitle}>No feedback yet.</p> : null}
          {feedback.map((item) => (
...
const styles = {
  ...
  sectionSubTitle: { color: "#f3d98b", margin: "4px 0 8px", fontSize: 15 },
  form: { marginTop: 18 },
  ...
  feedbackText: { color: "#ddd", whiteSpace: "pre-line", lineHeight: 1.5 },
};
