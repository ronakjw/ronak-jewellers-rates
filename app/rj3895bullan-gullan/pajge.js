...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(collection(db, "allowedUsers"), (snapshot) => {
      const users = snapshot.docs
        .map((docItem) => ({
          id: docItem.id,
          ...docItem.data(),
        }))
        .sort((a, b) => String(a.name || a.phone || a.id).localeCompare(String(b.name || b.phone || b.id)));
...
async function runAssistantCommand() {
  ...
  function updateAllowedUserDraft(field, value) {
    setNewAllowedUser((prev) => ({
      ...prev,
      [field]: value,
    }));
  }
  ...
  async function saveAllowedUser() {
    const phone = normalizeMobileNumber(newAllowedUser.phone);

    if (!phone) {
      setMessage("Enter a valid 10-digit mobile number for authorized user.");
      return;
    }
    ...
    try {
      await setDoc(
        ...
        {
          phone,
          name: String(newAllowedUser.name || "").trim(),
          firm: String(newAllowedUser.firm || "").trim(),
          city: String(newAllowedUser.city || "").trim(),
          role: String(newAllowedUser.role || "Dealer").trim(),
          notes: String(newAllowedUser.notes || "").trim(),
          active: true,
      ...
      setMessage("Authorized user saved.");
    } catch (err) {
      setMessage(err?.message ? `User save failed: ${err.message}` : "User save failed.");
    }
  }

  async function toggleAllowedUserStatus(phone, active) {
    try {
      await updateDoc(doc(db, "allowedUsers", phone), {
        active: !active,
        updatedAt: serverTimestamp(),
        updatedBy: user.email,
      });
  ...
  async function removeAllowedUser(phone) {
    const ok = window.confirm(`Remove ${phone} from authorized users?`);

    if (!ok) return;

    try {
      await deleteDoc(doc(db, "allowedUsers", phone));
      setMessage("Authorized user removed.");
    } catch (err) {
    ...
    Logout
        ...
        <section style={styles.dealerSection}>
          <div style={styles.sectionHeaderRow}>
            <div>
              <h2 style={styles.sectionTitle}>Authorized Users</h2>
              <p style={styles.subtitle}>Only these mobile numbers can access live rates by OTP.</p>
            </div>
          </div>
          ...
          <div style={styles.dealerGrid}>
            <div style={styles.controlCard}>
              ...
              <input
                style={styles.input}
                value={newAllowedUser.phone}
                inputMode="numeric"
                placeholder="10-digit number"
                onChange={(e) => updateAllowedUserDraft("phone", e.target.value)}
              />
            </div>
            ...
            <div style={styles.controlCard}>
              ...
              <input
                style={styles.input}
                value={newAllowedUser.name}
                onChange={(e) => updateAllowedUserDraft("name", e.target.value)}
              />
            </div>

            <div style={styles.controlCard}>
              <label style={styles.label}>Firm</label>
              <input
                style={styles.input}
                value={newAllowedUser.firm}
                onChange={(e) => updateAllowedUserDraft("firm", e.target.value)}
              />
            </div>
            ...
            <div style={styles.controlCard}>
              ...
              <input
                style={styles.input}
                value={newAllowedUser.city}
                onChange={(e) => updateAllowedUserDraft("city", e.target.value)}
              />
            </div>
            ...
            <div style={styles.controlCard}>
              ...
              <input
                style={styles.input}
                value={newAllowedUser.role}
                onChange={(e) => updateAllowedUserDraft("role", e.target.value)}
              />
            </div>
            ...
            <div style={styles.controlCard}>
              ...
              <input
                style={styles.input}
                value={newAllowedUser.notes}
                onChange={(e) => updateAllowedUserDraft("notes", e.target.value)}
              />
            </div>
          </div>

          <button type="button" style={styles.primaryButtonWide} onClick={saveAllowedUser}>
            Add / Update Authorized User
          </button>

          <div style={styles.dealerList}>
            {allowedUsers.length === 0 ? (
              <p style={styles.subtitle}>No authorized users added yet.</p>
            ) : (
              allowedUsers.map((item) => (
                <div key={item.id} style={styles.dealerRow}>
                  <div>
                    <strong style={styles.dealerName}>{item.name || item.phone || item.id}</strong>
                    <p style={styles.dealerMeta}>
                      📱 {item.phone || item.id}
                      {item.firm ? ` • ${item.firm}` : ""}
                      {item.city ? ` • ${item.city}` : ""}
                    </p>
                    <p style={styles.dealerMeta}>
                      ...
                      {item.lastLoginAt?.toDate ? ` • Last login: ${item.lastLoginAt.toDate().toLocaleString("en-IN")}` : ""}
                      {item.totalLogins ? ` • Logins: ${item.totalLogins}` : ""}
                    </p>
                  </div>

                  <div style={styles.dealerActions}>
                    <button
                      type="button"
                      style={item.active ? styles.blockButton : styles.activateButton}
                      onClick={() => toggleAllowedUserStatus(item.id, Boolean(item.active))}
                    >
                    ...
                    <button
                      ...
                      onClick={() => removeAllowedUser(item.id)}
                    >
                      Remove
                    </button>
                  </div>
