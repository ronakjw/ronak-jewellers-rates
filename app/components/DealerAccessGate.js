...
async function checkAccess({ phone, deviceId, firebaseIdToken }) {
  const res = await fetch("/api/access/check", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ phone, deviceId, firebaseIdToken }),
    cache: "no-store",
  });
...
export default function DealerAccessGate({
  ...
  useEffect(() => {
    ...
    async function restoreSession() {
      try {
        ...
        const access = await checkAccess({ phone, deviceId });
  ...
  async function grantAccess() {
    ...
    try {
      ...
      await logLogin({
        phone,
        deviceId,
        loginMethod: access.registeredNewDevice ? "first_device" : "known_device",
      });
  ...
  async function verifyOtp() {
    ...
    try {
      ...
      const deviceId = getOrCreateDeviceId();
      const access = await checkAccess({ phone, deviceId, firebaseIdToken });
