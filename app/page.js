      ...
      <table style={styles.compactTable}>
        <thead>
          <tr>
            ...
            <th style={styles.compactTh}>MCX Buy</th>
            <th style={styles.compactTh}>Buy Prem.</th>
            <th style={styles.compactTh}>Final Buy</th>
            <th style={styles.compactTh}>MCX Sell</th>
            <th style={styles.compactTh}>Sell Prem.</th>
            <th style={styles.compactTh}>Final Sell</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            <tr key={row.id}>
              <td style={styles.compactProductCell}>{row.title}</td>
              <td style={styles.compactTd}>
                ₹ {formatPrice(row.mcxBuy)}
                <MovementBadge movement={row.buyMovement} />
              </td>
              <td style={styles.compactTd}>{row.buyPremium === null ? "--" : formatPremium(row.buyPremium)}</td>
              <td style={styles.compactFinalTd}>₹ {formatPrice(row.finalBuying)}</td>
              <td style={styles.compactTd}>
                ₹ {formatPrice(row.mcxSell)}
                <MovementBadge movement={row.sellMovement} />
              </td>
              <td style={styles.compactTd}>{row.sellPremium === null ? "--" : formatPremium(row.sellPremium)}</td>
              <td style={styles.compactFinalTd}>₹ {formatPrice(row.finalSelling)}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
const styles = {
  ...
  compactTh: {
    padding: "12px 10px",
    borderBottom: "1px solid var(--rj-panel-border)",
    color: "var(--rj-muted)",
    fontSize: 12,
    textAlign: "left",
    textTransform: "uppercase",
