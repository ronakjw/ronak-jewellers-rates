  ...
  const unsub = onSnapshot(
    ...
    (snapshot) => {
      ...
      if (data?.active && Number.isFinite(expiresAt) && expiresAt > Date.now()) {
        volatilityUntilRef.current = Math.max(
          Number(volatilityUntilRef.current || 0),
          expiresAt
        );
        setVolatilityUntil((current) =>
          Math.max(Number(current || 0), expiresAt)
        );
        return;
      }

      // Do not clear a newer locally detected warning.
      if (Number(volatilityUntilRef.current || 0) <= Date.now()) {
        volatilityUntilRef.current = null;
        setVolatilityUntil(null);
      }
    },
    (error) => {
      console.error("Volatility listener failed", error);
    }
  );
...
if (
  ...
  useEffect(() => {
    if (!volatilityUntil || volatilityUntil > now.getTime()) return;
...
const isVolatilityActive = Boolean(
      ...
      try {
        ...
        if (Number.isFinite(currentBuyPrice) && currentBuyPrice > 0) {
          ...
          const updatedHistory = [
            ...
            { price: currentBuyPrice, time: nowMs },
          ].filter((item) => nowMs - item.time <= 40 * 1000);

          priceHistoryRef.current = updatedHistory;

          if (updatedHistory.length > 1) {
            const prices = updatedHistory.map((item) => item.price);
            const highest = Math.max(...prices);
            const lowest = Math.min(...prices);
            const movement = highest - lowest;
            const alreadyActive =
              Number(volatilityUntilRef.current || 0) > nowMs;
            ...
            if (movement >= 650 && settings?.volatilityWarningEnabled) {
              ...
              setVolatilityUntil((current) =>
                Math.max(Number(current || 0), warningUntil)
              );

              if (!alreadyActive && !volatilityLogInProgressRef.current) {
